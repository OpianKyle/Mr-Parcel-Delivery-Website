...
import { ArrowRight, Check, CircleHelp, FileText, Gauge, Info, Package, Plus, Trash2, Truck, X } from "lucide-react";
  ...
  const downloadQuote = () => {
    ...
    try {
      ...
      downloadQuotePdf({
        ...
        destinationCustomerName: form.destinationCustomerName,
        destinationAddress: form.destinationAddress,
        destinationPostalCode: form.destinationPostalCode,
        vatRate: pricing.vatRate,
        quoteReference: `MP-${new Date().toISOString().replace(/\D/g, "").slice(0, 14)}`,
        service: selectedQuote.service,
        lines: selectedQuote.lines,
        totalParcels: selectedQuote.totalParcels,
        chargeableWeightKg: selectedQuote.chargeableWeightKg,
        subtotalExVat: selectedQuote.subtotalExVat,
        vatAmount: selectedQuote.vatAmount,
        totalIncVat: selectedQuote.totalIncVat,
      });
    } catch {
      setDownloadError("We couldn't create the PDF. Please try again or use your browser's print option.");
    }
  };
